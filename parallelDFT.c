...
void fillArray(double *T, int n) {
     ...
     double val = PI / 180;
     for (i = 0; i < n; i++)
          T[i]=4*sin(2* (i * multp)) - 7 *sin(5 * (i * multp) );
...
int main(void) {
  double *points = create1DDoubleArray(40);
  fillArray(points, 40);
  printDoubleArray(points, 40);
  double complex *res = dft(points, 40);
  printComplexArray(res, 40);
  plotDouble("randValues.dat",points,40);
  double *absVals = findABS(res,40);
  printDoubleArray(absVals,40);
  plotDouble("absvalues.dat",absVals,40);
